...
import { IParametroBusca } from '../../interfaces/IParameroBusca';
...
const ListaRestaurantes = () => {
  ...
  useEffect(() => {
    verMais('http://localhost:8000/api/v1/restaurantes/')
  }, []);

  const verMais = (url: string, opcoes: AxiosRequestConfig = {}) => {
  ...
  const buscar = (evento: React.FormEvent<HTMLFormElement>) => {
    ...
    verMais('http://localhost:8000/api/v1/restaurantes/', opcoes)
  }
  ...
  return (<section className={style.ListaRestaurantes}>
    ...
    {<button onClick={() => verMais(paginaAnterior)} disabled={!paginaAnterior}>
      Página Anterior
    </button>}
    {<button onClick={() => verMais(proximaPagina)} disabled={!proximaPagina}>
      Próxima Página
    </button>}
  </section>)
}
